...
result = find_team_placement(2498, "LOUD")
...
# Compare multiple events
events_to_compare = [2498, 2498, 2498]  # Replace with actual event IDs
...
- Use `events.stages()` to list valid stage names before calling `events.standings()` with the `stage` parameter
