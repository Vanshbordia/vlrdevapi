...
<TypeTable type={{
  ...
  "players": {
    description: 'List of players on the team (name and id only). Always present; may be empty if the roster is unavailable.',
...
Each `Team` includes a `players` list with each player's `name` and `id`. This is useful for displaying full rosters per team. The `players` field is always present; it may be an empty list when the roster is unavailable, so always guard against that.

```python
import vlrdevapi

teams = vlrdevapi.event(2347).teams(stage="group-stage")
for stage_teams in teams:
    print(f"--- {stage_teams.stage_name} ---")
    for team in stage_teams.teams:
        print(team.name)
